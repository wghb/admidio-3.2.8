...
class HtmlPage {
    ...
    public function __construct($headline = '') {
        ...
        $this->sidebar = new HtmlSidebar('menu_main_script', $headline, $this);
        ...
        $this->addJavascriptFile('adm_program/libs/jquery-easing/jquery.easing.js');

        $this->addCssFile('adm_program/libs/scrolling-nav/css/scrolling-nav.css');
        $this->addJavascriptFile('adm_program/libs/scrolling-nav/js/scrolling-nav.js');
        $this->addJavascriptFile('http://cdnjs.cloudflare.com/ajax/libs/jquery-scrollTo/2.1.0/jquery.scrollTo.min.js');
    ...
    public function addDefaultMenu() {
        ...
        if ($gPreferences['enable_announcements_module'] == 1 || ($gPreferences['enable_announcements_module'] == 2 && $gValidLogin)) {
            $this->menu->addItem('menu_item_announcements', FOLDER_MODULES . '/announcements/announcements.php', $gL10n->get('ANN_ANNOUNCEMENTS'), 'announcements.png', 'right', 'menu_item_modules', 'admidio-default-menu-item');
        }
        if ($gPreferences['enable_download_module'] == 1) {
            $this->menu->addItem('menu_item_download', FOLDER_MODULES . '/downloads/downloads.php', $gL10n->get('DOW_DOWNLOADS'), 'download.png', 'right', 'menu_item_modules', 'admidio-default-menu-item');
        }
        if ($gPreferences['enable_mail_module'] == 1 && !$gValidLogin) {
            $this->menu->addItem('menu_item_email', FOLDER_MODULES . '/messages/messages_write.php', $gL10n->get('SYS_EMAIL'), 'email.png', 'right', 'menu_item_modules', 'admidio-default-menu-item');
        }
        ...
        if (($gPreferences['enable_pm_module'] == 1 || $gPreferences['enable_mail_module'] == 1) && $gValidLogin) {
            ...
            if ($unread > 0) {
                $this->menu->addItem('menu_item_private_message', FOLDER_MODULES . '/messages/messages.php', $gL10n->get('SYS_MESSAGES') . '<span class="badge">' . $unread . '</span>', 'messages.png', 'right', 'menu_item_modules', 'admidio-default-menu-item');
            } else {
                $this->menu->addItem('menu_item_private_message', FOLDER_MODULES . '/messages/messages.php', $gL10n->get('SYS_MESSAGES'), 'messages.png', 'right', 'menu_item_modules', 'admidio-default-menu-item');
            }
        }
        if ($gPreferences['enable_photo_module'] == 1 || ($gPreferences['enable_photo_module'] == 2 && $gValidLogin)) {
            $this->menu->addItem('menu_item_photo', FOLDER_MODULES . '/photos/photos.php', $gL10n->get('PHO_PHOTOS'), 'photo.png', 'right', 'menu_item_modules', 'admidio-default-menu-item');
        }
        if ($gPreferences['enable_guestbook_module'] == 1 || ($gPreferences['enable_guestbook_module'] == 2 && $gValidLogin)) {
            $this->menu->addItem('menu_item_guestbook', FOLDER_MODULES . '/guestbook/guestbook.php', $gL10n->get('GBO_GUESTBOOK'), 'guestbook.png', 'right', 'menu_item_modules', 'admidio-default-menu-item');
        }

        $this->menu->addItem('menu_item_lists', FOLDER_MODULES . '/lists/lists.php', $gL10n->get('LST_LISTS'), 'lists.png', 'right', 'menu_item_modules', 'admidio-default-menu-item');
        if ($gValidLogin) {
            $this->menu->addItem('menu_item_mylist', FOLDER_MODULES . '/lists/mylist.php', $gL10n->get('LST_MY_LIST'), 'mylist.png', 'right', 'menu_item_modules', 'admidio-default-menu-item');
        }

        if ($gPreferences['enable_dates_module'] == 1 || ($gPreferences['enable_dates_module'] == 2 && $gValidLogin)) {
            $this->menu->addItem('menu_item_dates', FOLDER_MODULES . '/dates/dates.php', $gL10n->get('DAT_DATES'), 'dates.png', 'right', 'menu_item_modules', 'admidio-default-menu-item');
        }

        if ($gPreferences['enable_weblinks_module'] == 1 || ($gPreferences['enable_weblinks_module'] == 2 && $gValidLogin)) {
            $this->menu->addItem('menu_item_links', FOLDER_MODULES . '/links/links.php', $gL10n->get('LNK_WEBLINKS'), 'weblinks.png', 'right', 'menu_item_modules', 'admidio-default-menu-item');
        }
        ...
        if ($gCurrentUser->isAdministrator() || $gCurrentUser->manageRoles() || $gCurrentUser->approveUsers() || $gCurrentUser->editUsers()) {
            ...
            if ($gCurrentUser->approveUsers() && $gPreferences['registration_mode'] > 0) {
                $this->menu->addItem('menu_item_registration', FOLDER_MODULES . '/registration/registration.php', $gL10n->get('NWU_NEW_REGISTRATIONS'), 'new_registrations.png', 'right', 'menu_item_administration', 'admidio-default-menu-item');
            }
            if ($gCurrentUser->editUsers()) {
                $this->menu->addItem('menu_item_members', FOLDER_MODULES . '/members/members.php', $gL10n->get('MEM_USER_MANAGEMENT'), 'user_administration.png', 'right', 'menu_item_administration', 'admidio-default-menu-item');
            }
            if ($gCurrentUser->manageRoles()) {
                $this->menu->addItem('menu_item_roles', FOLDER_MODULES . '/roles/roles.php', $gL10n->get('ROL_ROLE_ADMINISTRATION'), 'roles.png', 'right', 'menu_item_administration', 'admidio-default-menu-item');
            }
            if ($gCurrentUser->isAdministrator()) {
                $this->menu->addItem('menu_item_backup', FOLDER_MODULES . '/backup/backup.php', $gL10n->get('BAC_DATABASE_BACKUP'), 'backup.png', 'right', 'menu_item_administration', 'admidio-default-menu-item');
                $this->menu->addItem('menu_item_options', FOLDER_MODULES . '/preferences/preferences.php', $gL10n->get('SYS_SETTINGS'), 'options.png', 'right', 'menu_item_administration', 'admidio-default-menu-item');
            }
        }
    ...
    public function show($directOutput = true) {
        ...
        if ($this->showMenu) {
            ...
            if ($gValidLogin) {
                $this->sidebar->setLeftItems($this->menu->getLeftItems());
                $this->sidebar->setRightItems($this->menu->getRightItems());
                $htmlMenu .= $this->sidebar->show();
            }
        }
        ...
        $html .= '<div class="disabled-background disabled-background-disable"></div>';
        $html .= '<div class="admidio-theme-top">';
        $html .= $htmlMyBodyTop;
        $html .= '</div>'; // admidio-theme-top
        $html .= '<div class="admidio-content">';
        $html .= $htmlHeadline;
        $html .= $htmlMenu;
        $html .= $this->pageContent;
        $html .= '</div>'; // admidio-content
        $html .= '<div class="admidio-theme-bottom">';
